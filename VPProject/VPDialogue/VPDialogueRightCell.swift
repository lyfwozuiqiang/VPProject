...
class VPDialogueRightCell: UITableViewCell {
    ...
    private let borderGradientLayer:CAGradientLayer = {
        ...
        gradientLayer.colors = [UIColor(red: 0.33, green: 0.92, blue: 1.0, alpha: 1).cgColor,UIColor(red: 0.96, green: 0.31, blue: 0.98, alpha: 1).cgColor]
    ...
    private let shortLineShapeLayer = {
        let layer = CAShapeLayer.init()
        layer.strokeColor = UIColor(red: 0.33, green: 0.92, blue: 1.0, alpha: 1).cgColor
        layer.fillColor = UIColor.clear.cgColor
        return layer
    }()
    ...
    private let innerGradientLayer:CAGradientLayer = {
        ...
        layer.colors = [UIColor(red: 0.04, green: 0.04, blue: 0.1, alpha: 0.6).cgColor,UIColor(red: 0.04, green: 0.04, blue: 0.1, alpha: 1).cgColor]
        return layer
    }()
    ...
    override init(style: UITableViewCell.CellStyle, reuseIdentifier: String?) {
        ...
        labelContentView.leadingAnchor.constraint(equalTo: contentView.leadingAnchor, constant: 66).isActive = true
        labelContentView.trailingAnchor.constraint(equalTo: contentView.trailingAnchor, constant: -4).isActive = true
    ...
    override func layoutSubviews() {
        ...
        borderBezierPath.move(to: CGPoint(x: 15 + borderWidth/2, y: borderWidth/2))
        borderBezierPath.addLine(to: CGPoint(x: labelContentViewWidth - borderWidth/2, y: borderWidth/2))
        borderBezierPath.addLine(to: CGPoint(x: labelContentViewWidth - borderWidth/2 - 5, y: selfHeight - borderWidth/2))
        borderBezierPath.addLine(to: CGPoint(x: borderWidth/2, y: selfHeight - borderWidth/2))
        borderBezierPath.addLine(to: CGPoint(x: borderWidth/2 + 4, y: borderWidth/2 + 10.0))
        ...
        shortLineBezierPath.move(to: CGPoint(x: 3 + borderWidth/2, y: borderWidth/2))
        shortLineBezierPath.addLine(to: CGPoint(x: (borderWidth + 4)/2, y: selfHeight - borderWidth/2))
        let contatTransfrom = CGAffineTransformConcat(CGAffineTransform(scaleX: 1, y: 20/selfHeight), CGAffineTransformMakeTranslation(0, (selfHeight - 20)/2))
